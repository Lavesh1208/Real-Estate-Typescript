...
const Search = () => {
	...
	const {
		...
		isLoading: loading,
		refetch,
	} = useFilterListingQuery(searchQuery);
	...
	const onShowMoreClick = async () => {
		...
		const startIndex = numberOfListings !== undefined ? numberOfListings : 0;
		const urlParams = new URLSearchParams(location.search);
		urlParams.set("startIndex", startIndex.toString());
		const searchQuery = urlParams.toString();
		setSearchQuery(searchQuery);
		refetch();
		if (listings && listings.length < 9) {
			setShowMore(false);
		}
	};
	return (
